class PTHService {
    ...
    connectAndSetup(callback) { 
        this.notifyHumidity(callback);
        this.notifyPressure(callback);
        this.notifyTemperature(callback);
    };
    ...
    readTemperature(callback) {
        ...
        this.peripheral.readInt16LECharacteristic(PTHService.getUUIDs().Service, PTHService.getUUIDs().TemperatureCharacteristic, callback);
    };

    readHumidity(callback) {
        if(!this.hasHumidityChar()) return;

        this.peripheral.readUInt16LECharacteristic(PTHService.getUUIDs().Service, PTHService.getUUIDs().HumidityCharacteristic, callback);
    };

    readPressure(callback) {
        if(!this.hasPressureChar()) return;

        this.peripheral.readUInt32LECharacteristic(PTHService.getUUIDs().Service, PTHService.getUUIDs().PressureCharacteristic, callback);
    };
